...
import { createClient } from "@/lib/supabase/server"; // <--- Import pulito
...
export default async function RootLayout({
  ...
  const supabase = await createClient(); // <--- Niente più cookies() as any!
  const { data: { user } } = await supabase.auth.getUser();

  return (
    <html lang="it">
      <body className={`${inter.className} bg-gray-900 text-white min-h-screen flex flex-col`}>
        {/* ... (La navbar e il footer rimangono UGUALI a prima) ... */}
        ...
        <nav className="w-full bg-gray-950/80 backdrop-blur-md border-b border-gray-800 p-4 sticky top-0 z-50">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              <Link href="/contribuisci" className="hidden md:block text-sm font-semibold text-gray-300 hover:text-white">
                Contribuisci
              </Link>
