...
export default async function LocaleLayout({ children, params }: Props) {
  ...
  // Queste chiamate sono veloci e ottimizzate da next-intl
  const t = await getTranslations('Navbar');
  const messages = await getMessages();

  // NOTA: Abbiamo rimosso 'await supabase.auth.getUser()'.
  // Il sito ora carica istantaneamente.

  return (
    <html lang={locale} className="dark">
      <body 
        // Importante per evitare errori con estensioni browser
      ...
      >
        <NextIntlClientProvider messages={messages}>
          ...
          {/* Sfondo Statico Ottimizzato (Nessun calcolo CPU = 60fps fissi) */}
            ...
            <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
              ...
              <div className="flex items-center gap-6">
                <Link href="/" className="flex items-center gap-2 group">
                  <div className="w-8 h-8 rounded bg-gradient-to-br from-neon-pink to-neon-cyan flex items-center justify-center text-background font-bold font-display text-lg shadow-[0_0_15px_rgba(255,42,109,0.3)]">I</div>
                  <span className="text-xl font-bold font-display tracking-wide group-hover:text-neon-cyan transition-colors">IHearU</span>
                </Link>
              ...
              <div className="hidden md:flex gap-8 text-sm font-medium text-gray-300 font-sans absolute left-1/2 transform -translate-x-1/2">
                <Link href="/progetto" className="hover:text-neon-cyan transition-colors">{t('mission')}</Link>
                <Link href="/community" className="hover:text-neon-yellow transition-colors">{t('community')}</Link>
              </div>

              <div className="flex items-center gap-4">
                <Link href="/contribuisci" className="hidden md:block text-sm font-bold font-display text-neon-pink border border-neon-pink/30 px-4 py-2 rounded-full hover:bg-neon-pink/10 hover:border-neon-pink transition-all">
                  {t('contribute')}
                </Link>
                {/* AuthButton ora è autonomo */}
                <AuthButton /> 
              </div>
