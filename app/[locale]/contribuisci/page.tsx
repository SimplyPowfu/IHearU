...
const AngleIllustration = ({ angle, label, color }: { angle: string, label: string, color: string }) => {
  ...
  if (angle === 'left') { rotation = 'rotate-[335deg]'; position = 'top-1 left-5'; } 
  else if (angle === 'right') { rotation = '-rotate-[335deg]'; position = 'top-1 right-5'; }
...
export default function ContribuisciPage() {
  ...
  useEffect(() => {
    const initPage = async () => {
      ...
      const { data: wordsData } = await supabase.from('words').select('id, text').order('text');
      setWords(wordsData || []);
  ...
  const handleWebcamRecording = (recordedFile: File) => setFiles((prev) => [...prev, recordedFile]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const fileToUpload of files) {
        ...
        if (uploadError) {
          console.error(`Errore upload ${fileToUpload.name}:`, uploadError);
          continue; 
        }
  ...
  return (
    <main className="min-h-screen bg-background text-white py-20 px-6 flex justify-center relative overflow-hidden">
      ...
      {/* SFONDO STATICO (Pink Theme) */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        {/* Pink Centrale - Statico */}
        <div className="absolute top-[-10%] left-1/2 -translate-x-1/2 w-[800px] h-[600px] bg-neon-pink/10 rounded-full blur-[120px]" />
        {/* Cyan Accento - Statico */}
        <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-neon-cyan/5 rounded-full blur-[120px]" />
      </div>
      ...
      <div className="max-w-2xl w-full relative z-10 animate-fade-in-up">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-400 text-lg font-sans">{t('header.subtitle')}</p>
        </div>
        ...
        <Card className="mb-8 border-neon-cyan/20 bg-neon-cyan/5 backdrop-blur-md overflow-hidden relative">
          <div className="p-6">
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-neon-cyan/20 rounded-lg text-neon-cyan"><Info className="w-6 h-6" /></div>
              <div>
                <h3 className="text-lg font-bold text-white font-display">{t('instructions.title')}</h3>
                <p className="text-xs text-gray-400">{t('instructions.desc')}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-black/20 rounded-xl p-4 flex flex-col items-center text-center border border-white/5 hover:border-neon-cyan/30 transition-colors">
                <div className="mb-4"><AngleIllustration angle="left" label={t('instructions.angles.left_label')} color="cyan" /></div>
                <p className="font-bold text-white text-sm mb-1">{t('instructions.angles.left_title')}</p>
                <p className="text-xs text-gray-400">{t('instructions.angles.left_desc')}</p>
              </div>

              <div className="bg-black/20 rounded-xl p-4 flex flex-col items-center text-center border border-neon-cyan/30 bg-neon-cyan/5 shadow-lg scale-105 z-10">
                <div className="mb-4"><AngleIllustration angle="center" label={t('instructions.angles.center_label')} color="cyan" /></div>
                <p className="font-bold text-white text-sm mb-1">{t('instructions.angles.center_title')}</p>
                <p className="text-xs text-gray-400">{t('instructions.angles.center_desc')}</p>
              </div>

              <div className="bg-black/20 rounded-xl p-4 flex flex-col items-center text-center border border-white/5 hover:border-neon-cyan/30 transition-colors">
                <div className="mb-4"><AngleIllustration angle="right" label={t('instructions.angles.right_label')} color="cyan" /></div>
                <p className="font-bold text-white text-sm mb-1">{t('instructions.angles.right_title')}</p>
                <p className="text-xs text-gray-400">{t('instructions.angles.right_desc')}</p>
              </div>
            </div>
        ...
        <Card className="border-white/10 bg-surface/60 backdrop-blur-xl hover:border-neon-pink/30 transition-colors">
          <form className="space-y-8" onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
            ...
            <div className="space-y-3">
              <label className="block text-sm font-bold text-gray-300 uppercase tracking-wider">{t('form.select_label')}</label>
              <div className="relative group">
                ...
                >
                  ...
                  {words.map((word) => <option key={word.id} value={word.id} className="bg-surface">{word.text}</option>)}
                </select>
                <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400 group-hover:text-white transition-colors">▼</div>
              </div>
            </div>
            ...
            <div className="bg-background/50 p-1 rounded-xl flex border border-white/10">
              <button
                ...
                className={`flex-1 py-3 px-4 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'upload' ? 'bg-surfaceHighlight text-neon-pink shadow-lg border border-neon-pink/20' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5'}`}
              >
                <UploadCloud className="w-4 h-4" /> {t('form.tabs.upload')}
              </button>
              <button
                type="button"
                onClick={() => setActiveTab('webcam')}
                className={`flex-1 py-3 px-4 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'webcam' ? 'bg-surfaceHighlight text-neon-pink shadow-lg border border-neon-pink/20' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5'}`}
              >
                <Video className="w-4 h-4" /> {t('form.tabs.webcam')}
              </button>
            </div>
              ...
              {activeTab === 'upload' ? (
                <div className="animate-fade-in h-full">
                  <div className="relative h-64 border-2 border-dashed border-white/20 rounded-xl flex flex-col items-center justify-center text-center hover:border-neon-pink/50 hover:bg-neon-pink/5 transition-all cursor-pointer group">
                    ...
                    <p className="text-gray-300 font-medium group-hover:text-white">{t('form.dropzone.title')}</p>
                ...
                <div className="animate-fade-in w-full">
                  ...
                  <p className="text-center text-xs text-gray-400 mt-2">{t('form.webcam_note')}</p>
                </div>
              )}
            </div>
              ...
              <div className="bg-surface/40 rounded-xl p-4 border border-white/10 animate-fade-in">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-sm font-bold text-white flex items-center gap-2">
                    <CheckCircle className="w-4 h-4 text-green-400" /> {t('queue.title')} ({files.length})
                  </h4>
                  <button type="button" onClick={() => setFiles([])} className="text-xs text-red-400 hover:text-red-300 hover:underline flex items-center gap-1">
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-white/10">
                  {files.map((f, index) => (
                    <div key={index} className="flex items-center justify-between bg-black/20 p-3 rounded-lg border border-white/5 group hover:border-white/20 transition-all">
                      ...
                      <button type="button" onClick={() => removeFile(index)} className="p-2 hover:bg-white/10 rounded-full text-gray-500 hover:text-red-400 transition-colors">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
            ...
            <div className="bg-surfaceHighlight/50 p-4 rounded-xl border border-white/5 flex gap-4 items-start group hover:border-neon-pink/30 transition-colors">
              <div className="pt-1">
                <input type="checkbox" checked={acceptedTerms} onChange={(e) => setAcceptedTerms(e.target.checked)} className="w-5 h-5 rounded border-gray-600 bg-surface text-neon-pink focus:ring-neon-pink cursor-pointer accent-neon-pink" />
              </div>
              <div className="text-xs text-gray-400 leading-relaxed">
                <strong className="text-gray-200 block mb-1">{t('terms.label')}</strong>
                {t('terms.text_part1')} <span className="text-neon-pink font-bold"> {t('terms.text_highlight')}</span> {t('terms.text_part2')}
              </div>
            </div>

            <div className="pt-2">
              <Button variant="neon-pink" size="lg" type="submit" disabled={loading || files.length === 0} className="w-full shadow-lg shadow-neon-pink/20 disabled:opacity-50 disabled:cursor-not-allowed">
                {loading ? t('submit.loading') : files.length > 0 ? t('submit.button', { count: files.length }) : t('submit.button_single')}
              </Button>
            </div>
